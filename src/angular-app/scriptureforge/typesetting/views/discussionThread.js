...
angular.module('typesetting.discussionThread',
    ['ui.bootstrap', 'bellows.services', 'ngAnimate', 'palaso.ui.notice', 'typesetting.discussionServices'])
// controller for discussionThread
.controller('discussionThreadCtrl', [

        '$scope',
        '$state',
        'typesettingDiscussionService',
        '$location', 'sessionService', 'modalService', 'silNoticeService',
function($scope, $state, discussionService, $location, sessionService, modal, notice) {

  // path is "/discussion/:threadId"
  var threadId = '56d231b91b5c6c350beb946b';
    ...
    discussionService.getPageDto(function(result) {
      ...
      for (var i = 0; i < $scope.discussion.threads.length; i++) {
        if ($scope.discussion.threads[i].id === threadId) {
          $scope.discussion.currentThreadIndex = 1;
          break;
        }
      }
      ...
      if ($scope.discussion.currentThreadIndex < 0) {
        ...
        $scope.thread = $scope.discussion.threads[1];
      }
    });
  } else {
    $scope.thread = $scope.discussion.threads[1];
